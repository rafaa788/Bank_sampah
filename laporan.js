...
// ==================== GENERATE HTML EXCEL ADMIN DENGAN REKAP SAMPAH ====================
function generateAdminExcelHTML(rekapData, allData, filterBSU, filterRW, filterRT, periode) {
    ...
    for (var bsu in bsuGroups) {
        ...
        // Ringkasan per nasabah dengan 3 FOTO
        html += '<table>';
        html += '<tr>';
        html += '<th style="width:5%;">No</th>';
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            // Foto Timbang
            if (item.fotoTimbang) {
                html += '<td class="foto-cell"><img src="' + item.fotoTimbang + '" class="foto-img" onerror="this.parentElement.innerHTML=\'<span style=color:#999;font-size:10px;>Tidak ada</span>\'"><br><span class="foto-label">Foto Timbang</span></td>';
            } else {
                html += '<td class="foto-cell" style="color:#999;font-size:10px;">Tidak ada</td>';
            }
            
            // Foto Hasil
            if (item.fotoHasil) {
                html += '<td class="foto-cell"><img src="' + item.fotoHasil + '" class="foto-img" onerror="this.parentElement.innerHTML=\'<span style=color:#999;font-size:10px;>Tidak ada</span>\'"><br><span class="foto-label">Foto Hasil</span></td>';
            } else {
                html += '<td class="foto-cell" style="color:#999;font-size:10px;">Tidak ada</td>';
            }
            ...
            // Foto Bukti
            if (item.fotoBukti) {
                html += '<td class="foto-cell"><img src="' + item.fotoBukti + '" class="foto-img" onerror="this.parentElement.innerHTML=\'<span style=color:#999;font-size:10px;>Tidak ada</span>\'"><br><span class="foto-label">Foto Bukti</span></td>';
            } else {
                html += '<td class="foto-cell" style="color:#999;font-size:10px;">Tidak ada</td>';
            }
        ...
        // Detail per nasabah dengan 3 FOTO
        for (var i = 0; i < items.length; i++) {
            var item = items[i];
            ...
            for (var j = 0; j < item.detail.length; j++) {
                ...
                // Foto Timbang
                if (d.fotoTimbang) {
                    html += '<td class="foto-cell"><img src="' + d.fotoTimbang + '" class="foto-img" onerror="this.parentElement.innerHTML=\'<span style=color:#999;font-size:9px;>Tidak ada</span>\'"></td>';
                } else {
                    html += '<td class="foto-cell" style="color:#999;font-size:9px;">-</td>';
                }
                
                // Foto Hasil
                if (d.fotoHasil) {
                    html += '<td class="foto-cell"><img src="' + d.fotoHasil + '" class="foto-img" onerror="this.parentElement.innerHTML=\'<span style=color:#999;font-size:9px;>Tidak ada</span>\'"></td>';
                } else {
                    html += '<td class="foto-cell" style="color:#999;font-size:9px;">-</td>';
                }
                ...
                // Foto Bukti
                if (d.fotoBukti) {
                    html += '<td class="foto-cell"><img src="' + d.fotoBukti + '" class="foto-img" onerror="this.parentElement.innerHTML=\'<span style=color:#999;font-size:9px;>Tidak ada</span>\'"></td>';
                } else {
                    html += '<td class="foto-cell" style="color:#999;font-size:9px;">-</td>';
                }
...
window.exportAdminLaporanExcelFull = function() {
    ...
    showToast('Laporan Admin Excel berhasil diekspor!', false);
};

// --- MINGGUAN ---
window.exportLaporanMingguanExcel = function() {
    var laporan = generateLaporanMingguanAdmin(daftarSampah, currentFilterBSU, currentFilterRW, currentFilterRT);
...
window.exportLaporanBulananExcel = function() {
    var laporan = generateLaporanBulananAdmin(daftarSampah, currentFilterBSU, currentFilterRW, currentFilterRT);
    if (laporan.data.length === 0) { showToast('Tidak ada data untuk periode ini!', true); return; }
    var rekapData = generateRekapPerNasabahAdmin(laporan.data);
...
window.exportLaporanTahunanExcel = function() {
    var laporan = generateLaporanTahunanAdmin(daftarSampah, currentFilterBSU, currentFilterRW, currentFilterRT);
    if (laporan.data.length === 0) { showToast('Tidak ada data untuk periode ini!', true); return; }
    var rekapData = generateRekapPerNasabahAdmin(laporan.data);
...
function generateAdminPDFHTML(rekapData, allData, filterBSU, filterRW, filterRT, periode) {
    ...
    for (var bsu in bsuGroups) {
        ...
        html += '<table>';
        html += '<tr><th>No</th><th>Nama Nasabah</th><th>RW/RT</th><th>Berat (kg)</th><th>Transaksi</th><th>Tabungan</th><th>Foto</th></tr>';
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            var fotoText = '-';
            var fotoList = [];
            if (item.fotoTimbang) fotoList.push('');
            if (item.fotoHasil) fotoList.push('');
            if (item.fotoBukti) fotoList.push('');
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            html += '<table>';
            html += '<tr><th>No</th><th>Tanggal</th><th>Nama Sampah</th><th>Berat</th><th>Harga</th><th>Nilai</th><th>Foto</th></tr>';
            
            for (var j = 0; j < item.detail.length; j++) {
                var d = item.detail[j];
                var fotoText = '-';
                var fotoList = [];
                if (d.fotoTimbang) fotoList.push('');
                if (d.fotoHasil) fotoList.push('');
                if (d.fotoBukti) fotoList.push('');
                if (fotoList.length > 0) fotoText = fotoList.join(' ');
...
window.exportAdminNasabahRekap = function() {
    ...
    html += '.nasabah-name { text-align: left; font-weight: bold; color: #1a237e; }';
    html += '.foto-cell { text-align: center; }';
    html += '.foto-img { max-width: 60px; max-height: 60px; border-radius: 4px; border: 1px solid #ddd; }';
    ...
    html += '<table>';
    html += '<tr><th>No</th><th>Nama Nasabah</th><th>BSU</th><th>RW/RT</th><th>Total Berat</th><th>Total Transaksi</th><th>Total Tabungan</th><th>📷 Timbang</th><th>📸 Hasil</th><th>📋 Bukti</th></tr>';
    ...
    for (var i = 0; i < rekapData.length; i++) {
        ...
        // Foto Timbang
        if (n.fotoTimbang) {
            html += '<td class="foto-cell"><img src="' + n.fotoTimbang + '" class="foto-img" onerror="this.parentElement.innerHTML=\'<span style=color:#999;font-size:9px;>-</span>\'"></td>';
        } else {
            html += '<td class="foto-cell" style="color:#999;">-</td>';
        }
        
        // Foto Hasil
        if (n.fotoHasil) {
            html += '<td class="foto-cell"><img src="' + n.fotoHasil + '" class="foto-img" onerror="this.parentElement.innerHTML=\'<span style=color:#999;font-size:9px;>-</span>\'"></td>';
        } else {
            html += '<td class="foto-cell" style="color:#999;">-</td>';
        }
        ...
        // Foto Bukti
        if (n.fotoBukti) {
            html += '<td class="foto-cell"><img src="' + n.fotoBukti + '" class="foto-img" onerror="this.parentElement.innerHTML=\'<span style=color:#999;font-size:9px;>-</span>\'"></td>';
        } else {
            html += '<td class="foto-cell" style="color:#999;">-</td>';
        }
...
console.log('✅ Laporan Admin dengan Rekap Per Nama Sampah dan 3 Foto Bukti loaded!');
